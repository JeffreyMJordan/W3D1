...
def neighbors_of_certain_b_countries
  ...
  execute(<<-SQL)
    ...
    FROM 
      countries 
    WHERE 
      continent IN ((SELECT continent FROM countries WHERE name='Belize'), (SELECT continent FROM countries WHERE name='Belgium'))
  SQL
end
...
def sparse_continents
  ...
  execute(<<-SQL)
    ...
    FROM 
      countries x 
    WHERE 
      25000000 > ALL(SELECT population FROM countries y WHERE x.continent = y.continent)
  SQL
end
