...
def cl_to_lr
  ...
  execute(<<-SQL)
    ...
    WHERE
      a.stop_id = 53
      AND b.stop_id = (SELECT id FROM stops WHERE name='London Road')
  SQL
end
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT 
      DISTINCT stopb.name, a.company, a.num 
    FROM 
      routes a 
    JOIN 
      routes b ON (a.company=b.company AND a.num=b.num)
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT 
      DISTINCT part1.num, part1.company, part1.stopname, part2.num, part2.company
    FROM 
      (SELECT
    ...
    JOIN (SELECT
          ...
          WHERE
            c.stop_id = 213) AS part2
          ON part1.bb=part2.dd
